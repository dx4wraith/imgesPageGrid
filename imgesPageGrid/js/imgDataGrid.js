...
(function (factory) {
    ...
    grid.prototype = {
        create: function () {
            ...
            str.push('                                <td> <input class="imgDataGrid-page-input form-control" type="text" value="0"> 共 <span class="imgDataGrid-totalPage">1</span> 页</td>');
            str.push('                                <td class="imgDataGrid-disabled"><span class="imgDataGrid-separator"></span></td>');
            ...
            page.find('.first_gridPager').on('click', function () {
                if ($this.isActivePageBtn(this)) {
                    $this.gotopage(1);
                }
            });
            page.find('.prev_gridPager').on('click', function () {
                if ($this.isActivePageBtn(this)) {
                    $this.gotopage($this.options.page.page - 1);
                }
            });
            ...
            page.find('.last_gridPager').on('click', function () {
                if ($this.isActivePageBtn(this)) {
                    $this.gotopage($this.options.page.total);
                }
            });
        ...
        gotopage: function (page) {
            var $this = this;
            if ($this.options.page.page == page) {
                return;
            }
        ...
        showLoading: function () {
            var $this = this;
            $this.$elem.find('.imgDataGrid-loading').show();
        },
        hideLoading: function () {
            var $this = this;
            $this.$elem.find('.imgDataGrid-loading').hide();
        },
        isActivePageBtn: function (o) {
            if ($(o).hasClass('imgDataGrid-disabled')) {
                return false;
            } else {
                return true
            }
        ...
        formatData: function (data) {
            ...
            if (data.rows.length == 1) {
                ...
                $.each(data.rows, function (index, item) {
                    strs.push('<li id="' + index + '">' + $this.options.render(item, index) + '</li>');
                })
                $this.$elem.find('.imgDataGrid-list ul').html(strs.join(''));
                $this.$elem.find('.imgDataGrid-list ul li ').css('width', 994).css('height', 500);
                ...
                $this.$elem.find('.imgDataGrid-list ul').html(strs.join(''));
                $this.$elem.find('.imgDataGrid-list ul li ').css('width', 470).css('height', 400);
                if ($this.options.onClick) {
                    $this.$elem.find('.imgDataGrid-list ul li ').on('click', function () {
                        var $li = $(this);
                        var index = parseInt($li.attr('id'));
                ...
                $this.$elem.find('.imgDataGrid-list ul').html(strs.join(''));
                $this.$elem.find('.imgDataGrid-list ul li ').css('width', $this.options.img.width).css('height', 270);
                if ($this.options.onClick) {
                    $this.$elem.find('.imgDataGrid-list ul li ').on('click', function () {
                        var $li = $(this);
                        var index = parseInt($li.attr('id'));
        ...
        loadData: function () {
            ...
            if ($this.options.data) {//本地存储
                ...
                $.ajax({
                    ...
                    data: $.param($.extend($this.defaults.page, $this.options.params)),
                    dataType: 'json',
